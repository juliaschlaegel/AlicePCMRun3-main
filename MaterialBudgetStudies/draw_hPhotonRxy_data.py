...
def draw_hPhotonRxy_data(filename_data, filename_mc, cutname, suffix, folder, date):
    ...
    h1ev_data = list_ev_after.Get("hCollisionCounter")# .Clone("h1ev");
    print("type h1ev_data: ", type(h1ev_data))
    nev_data = h1ev_data.GetBinContent(10);
    print("NEV data: ", nev_data)

    h1nch_data = list_ev_after.Get("hMultNTracksPV");
    nch_data = h1nch_data.GetMean();
    print("nch data: ", nch_data)

    h2_data = list_v0_data.Get("hGammaRxy")#("hGammaRxy_recalc");
    print("type h2_data: ", type(h2_data), h2_data)
    ...
    c1.SaveAs(outname);
    #c1.SaveAs("hPhotonRxy_data_circle")
