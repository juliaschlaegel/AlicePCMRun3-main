...
#________________________________________________
def draw_m_vs_phiv_with_cut(filename_data, suffix, folder, date):
    rootfile_data = TFile.Open(filename_data, "READ");
    rootdir_data = rootfile_data.Get("dielectron-qc");
    dir_pair = rootdir_data.Get("Pair")
    dir_same = dir_pair.Get("same")
    dir_uls = dir_same.Get("uls")


    h2_data = dir_uls.Get("hMvsPhiV");
    h2_data.Sumw2();
    h2_data.SetXTitle("#varphi_{v} (rad.)")
    h2_data.SetYTitle("m_{ee} (GeV/c^{2})")
    ...
    txt.AddText("m_{ee} vs. #varphi_{v}")
    txt.Draw();
    ...
    outname = os.path.join(folder, "{0}_mee_vs_phiv_{1}.pdf".format(date, suffix))
    c1.SaveAs(outname);

if __name__ == "__main__":
    filename_data = "/Users/juliaschlagel/Analysis240411/Analysis/HLtrains/data/AnalysisResults_Dalitz_qc_22o_225019.root"
    ...
    draw_m_vs_phiv_with_cut(filename_data, suffix, folder, date);