...
# This code was modifies by Julia Schlägel (July 2024)
...
def draw_comparison_parameters(list_data, list_mc, index, cutname, date, period_str):
        ymin = [6*1e-3, 125*1e-3,  10*1e-4, 1e-8]
        ymax = [22*1e-3, 138*1e-3 , 110*1e-4, 1e-3]
...
def run(filename_data, filename_mc,config,type,folder, period_str):
    ...
    list_data = [list_data_lambda, list_data_mean, list_data_sigma, yield_list_data]
    list_mc = [list_mc_lambda, list_mc_mean, list_mc_sigma, yield_list_mc]

    print("length", len(list_data), (len(list_data_lambda,)), (len(list_data_mean)), (len(list_data_sigma)), len(list_parameters_data))
    print("length", len(list_mc), (len(list_mc_lambda,)), (len(list_mc_mean)), (len(list_mc_sigma)), len(list_parameters_mc))
    for i in range(4):
        draw_comparison_parameters(list_data[i], list_mc[i], i,list_data[i][0].GetTitle(), date, period_str)
