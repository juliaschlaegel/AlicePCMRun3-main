...
def run(filename, config, typ, suffix, folder):
    ...
    if config["common"]["do_ptspectrum"] == True:
        if run_type == "one":
            ...
            if typ == "mc":
                if decay == "dalitz":
                    ana_pi0 = PairAnalyzer(meson, filename, "pi0eta-to-gammagamma-mc-pcmdalitzee", typ, filename_mc, config, decay);
                if decay == "pcm":
                    ana_pi0 = PairAnalyzer(meson, filename, "pi0eta-to-gammagamma-mc-pcmpcm", typ, filename_mc, config, decay)
            else:
                if decay == "dalitz":
                    ana_pi0 = PairAnalyzer(meson, filename, "pi0eta-to-gammagamma-pcmdalitzee", typ, filename_mc, config, decay);
                if decay == "pcm":
                    ana_pi0 = PairAnalyzer(meson, filename, "pi0eta-to-gammagamma-pcmpcm", typ, filename_mc, config, decay);
        ...
        if run_type == "both_types":
            ...
            for isys in range(0, nsys):
                ...
                for ic in range(0,nc):
                    ...
                    #print("Raw yield data geladen als:")
                    # for i in range(1, raw_data.GetNbinsX() + 1):
                    #     bin_content_d = raw_data.GetBinContent(i)
                    #     bin_center_d = raw_data.GetBinCenter(i)
                    #     print(f"Bin {i}: Center = {bin_center_d}, Content = {bin_content_d}")
                    corr_class.plot_pythia(utils_mc, TString(pythia_name), "true")
                    ...
                    corr = corr_class.calc_corr_yield_vs_pt(TString(corr_name), "true", raw_data, eff, acc, acc_class)
                    if decay == "pcm":
                    ...
                    if ic == (len(cutnames)-1):
                        # print("#######Länge liste eff_dalitz", len(eff_list_dalitz), eff_list_dalitz, len(eff_list_pcm))
                        # print("#######Länge Liste corr_dalitz", len(corr_list_dalitz),corr_list_dalitz, len(corr_list_pcm)) 
                        # print("Länge der akzeptanz: (dalitz und pcm)", len(acc_list_dalitz), len(acc_list_pcm))
                        # if decay == "dalitz":
                        ...
                        if decay == "dalitz":
                            ...
                            comp_class.compare_cuts(eff_list_dalitz, cutnames, "dalitz", filename_mc, config, TString(comp_name_cuts_eff), "eff", utils)
                            ...
                            comp_class.compare_cuts(eff_list_pcm, cutnames, "pcm", filename_mc, config, TString(comp_name_cuts_eff), "eff", utils)
                    ...
                    comp_class = Compare(filename_mc)
                    comp_class.plot_acc_dalitz_vs_pcm(filename_mc, config, TString(acc_name_comp), "true", utils)
                    comp_class.plot_eff_dalitz_vs_pcm(filename_mc, config, raw_mc_dalitz, raw_mc_pcm, TString(eff_name_comp),"true")
                    ...
                    if decay == "dalitz":
                        ...
                        comp_class.compare_13tev(filename_comp_dalitz, "eff", eff_pc, "dalitz",TString(comp_eff_13Tev_dalitz_name), "true")
                        comp_class.compare_13tev(filename_comp_dalitz, "corr", corr, "dalitz", TString(comp_corr_13Tev_dalitz_name), "true")
                    if decay == "pcm":
                        comp_class.compare_13tev(filename_comp_pcm, "acc", acc_pc, "pcm", TString(comp_acc_13Tev_pcm_name) , "true")
                        comp_class.compare_13tev(filename_comp_pcm, "eff", eff_pc, "pcm", TString(comp_eff_13Tev_pcm_name), "true")
                    ...
                    comp_class.plot_eff_times_acc(filename_mc, config, raw_mc_dalitz, raw_mc_pcm, TString(comp_eff_times_acc_name), "true", utils)
...
period_array = [ "LHC22o_pass6_small", "LHC24b1"]
...
for dec in range(len(decay_array)):
    ...
    for r in range(len(run_type_array)):
        ...
        for i in range(len(period_array)):
            ...
            folder = "/Users/juliaschlagel/Analysis240411/Analysis/AlicePCMRun3-main/analysis_results_test"
