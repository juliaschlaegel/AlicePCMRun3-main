...
class PairAnalyzer:
    ...
    def calculate_raw_yield_MC(self, histo, params, params_err, binwidth, nev):
        ...
        error_integral = c_double(0.0)
        integral_histo = histo.IntegralAndError(integral_min, integral_max, error_integral) #without "width"
        for i in range (integral_min, integral_max):
            print("###### bin content ", histo.GetBinContent(i))
        error = error_integral.value /binwidth / nev
        raw_yield = (integral_histo)/binwidth /nev
        print("raw yield", raw_yield, integral_histo, "bin width", binwidth, "error", error)
        return raw_yield, error
    ...
    def set_2D_histograms(self, outlist):
        ...
        if self.list_pair:
            print("list_ev: ", self.list_pair)
            h2mix_help = self.list_pair.Get("mix")
            if h2same_help:
                print("mix was loaded")
                h2mix = h2mix_help.Get("hMggPt").Clone("h2mix")
                if h2same:
                    print("h2mix loaded", type(h2mix))
        else:
            print("list_pair_ss is not valid.")
    ...
    def analyze_ptspectrum(self, start_pT):
        outlist = THashList();
        ...
        for i in range(start_pT, npt-1):
            ...
            if self.initial_params[0] == 0: # amplitude 0 -> did not yet assign values in config
                ...
                f1total.SetParLimits(2, 0.7 * sigma_init, 1.5*sigma_init);#0.9
                f1total.SetParLimits(3,0.7 * lambda_init, 1.6*lambda_init);#0.9

            h1fit = h1prepared.Clone("h1mgg_fitted_pt{0}".format(i));
            h1fit.Fit(f1total,"QRME","",fit_min, fit_max); #RME
