...
class PlotRawYieldInvMass:
    ...
    def __init__(self, decay, meson, filename, dirname):
    ...
    def PlotHistoYield(self, fHistoParameter,
                            ...
                            numberColumnsPlot,
                            fDecayChannel, fMonteCarloInfo, ssnames, decayChannel = "#gamma#gamma", fDetectionChannel = "#gamma#gamma",
        ...
        if len(fHistoParameter) != 1:     
            ...
            p1 = c1.cd()
            p1.SetPad(0,0.1,1,1);
            p1.SetMargin(0.25,0.1,0.2,0.15);

        p1.SetTicks(1,1);
        p1.SetLogy(); 
        

        frame1 = p1.DrawFrame(0., yMin_, 15., yMax_);
        if self.meson == "pi0":
            FrameSettings(frame1, "p_{T} (GeV/c)", "#frac{1}{#it{N}_{ev}}#frac{d#it{N}^{#pi^{0}}}{d#it{p}_{T}} counts #upoint (GeV/#it{c})^{-1}",0., 15.)
        if self.meson == "eta":
            FrameSettings(frame1, "p_{T} (GeV/c)", "#frac{1}{#it{N}_{ev}}#frac{d#it{N}^{#eta}}{d#it{p}_{T}} counts #upoint (GeV/#it{c})^{-1}",0., 15.)
        frame1.GetXaxis().SetTitleSize(0.045);
        frame1.GetYaxis().SetTitleSize(0.045);
        #rame1.GetXaxis().SetTitle("pT")
        ...
        for icut in range(len(fHistoParameter)):
            ...
            fHistoParameter[icut].SetXTitle("pT")
            #print("Listeneintrag:", icut, "fHisto: ", fHistoParameter[icut], ssnames)
        ...
        if len(fHistoParameter) != 1:
            ...
            p2.SetTicks(1,1);
            p2.SetLogy();
            #p2.SetRangeUser(0.5, 1.5)
            frame2 = p2.DrawFrame(0., 0.8, 15., 2.0);
            FrameSettings(frame2, "p_{T} (GeV/c)", "#frac{cut}{analysis}",0., 15.)
            ...
            for iratio in range(len(fHistoParameter)-1):
                ...
                h1ratio1.Reset();
                h1ratio1.Sumw2();
                h1ratio1.GetYaxis().SetRangeUser(0.5, 1.5)
        ...
        # else:
        #     txt = TPaveText(0., 0.8, 1.0, 0.9,"NDC")
        ...
        if len(fHistoParameter) != 1:
            ...
            txt = TPaveText(0.65, 0.72, 0.95, 0.82, "NDC")
        txt.SetFillColor(kWhite);
        txt.SetFillStyle(0);
        txt.SetBorderSize(0);
        ...
        if self.meson == "eta":
            if self.decay == "pcm":
                txt.AddText("\eta \\rightarrow \gamma \gamma")
            if self.decay == "dalitz":
                txt.AddText("\eta \\rightarrow e^{+} e^{-} \gamma")
        if self.meson =="pi0":
            if self.decay == "pcm":
                txt.AddText("\pi^{0} \\rightarrow \gamma \gamma")
            if self.decay == "dalitz":
                txt.AddText("\pi^{0} \\rightarrow e^{+} e^{-} \gamma")
        txt.Draw();
        ROOT.SetOwnership(txt,False);

        leg = TLegend(0.45,0.62,0.95,0.77);
        leg.SetBorderSize(0);
        ...
        for icut in range(len(fHistoParameter)):
            ...
            leg.AddEntry(fHistoParameter[icut],"{}".format(ssnames[icut]),"ep");
        leg.Draw("");
