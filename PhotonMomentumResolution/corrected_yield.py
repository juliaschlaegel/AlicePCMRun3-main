...
class CorrectedYield:
    ...
    #get canvas and pad to plot the corrected yield in 
        ...
        h_corrected.SetMarkerSize(0.85)
    ...
    def plot_pythia(self, utils, name_plot, save): #This is the corrected yield we would expect from MC
        ...
        leg.SetFillStyle(0)
        leg.SetTextSize(0.03)
        leg.AddEntry(h1, "corrected yield Pythia", "LP")
        leg.Draw("")
        ROOT.SetOwnership(leg,False)
    ...
    def compare_pythia_corr_yield(self, corrected_hist, pythia_hist, decay, name_plot, save):
        ...
        max_h2 = pythia.GetMaximum()
        max_y = 1.50 * max(max_h1, max_h2)  
    ...
    def plot_fit_func(self, corr_hist_scaled, decay, name_plot, save):
        ...
        #utils.scale_by_pt(h_corr)
        h_corr.Scale(57.8*1e-3) #getting the differential invariant yield
        h_corr.Scale(1e12) #scaling to pico barn
        
        A_e = 244 * 1e9#1.63e-11 #536e-21
        M_pi = 0.135
        T_e = 0.157
        A_init = 27 * 1e9 #0.693 #8.4 #30e-21
        T_init = 0.6  #0.63
        n = 2.96
        ...
        f1TCM_high = TF1("TCM_high","[0] * (TMath::Power((1+(TMath::Power(x,2))/(TMath::Power([1],2)*[2])), -[2]))", 4., 15.,3)
        f1TCM_high.SetParameters(A_init, T_init, n)
        f1TCM.SetParameters(A_e, M_pi, T_e, A_init, T_init, n)
        f1TCM.SetLineColor(ROOT.kBlack)
        f1TCM_high.SetLineColor(ROOT.kOrange)
        ...
        #h_corr.Fit(f1TCM_high,"TCM_h","",4,15)
        y = h_corr.GetYaxis() 
        ...
        # params = f1TCM_high.GetParameters()
        # for i in range(3):
    ...
    def plot_fit_func_13Tev(self, filename_mc, filename_comp, decay, name_plot, save):
        ...
        #h_corr_13 = comp.get_13tev_file(filename_comp_pcm, "InvCrossSectionPi0")

        
        h_corr_13.Scale(57.8*1e-3) #getting the differential invariant yield
        h_corr_13.Scale(1e12) #scaling to pico barn
        
        A_e = 536 * 1e9#1.63e-11 #536e-21
        M_pi = 0.135
        T_e = 0.142
        A_init = 30 *1e9#0.693 #8.4 #30e-21
        T_init = 0.63  #0.63
        n = 2.96
        #m = 0.26 *1e-3
        ...
        #f1TCM_13.SetParLimits(6, 0.5 * m, 1.5 * m)
