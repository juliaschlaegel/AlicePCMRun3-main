...
class Compare:
    ...
    def plot_eff_dalitz_vs_pcm(self, filename_mc, config, raw_yield_mc_dalitz, raw_yield_mc_pcm, name_plot, save):
        utils_dalitz = Utility(filename_mc, config, "dalitz")
        utils_pcm = Utility(filename_mc, config, "pcm")
        ...
        max_y = 1.5 * max(max_h1, max_h2)  # 50% higher than the biggest point
    ...
    def compare_corr_dalitz_pcm(self, corr_dalitz, corr_pcm, name_plot, save): 
        ...
        # Add legend
        legend1 = ROOT.TLegend(0.2, 0.35, 0.6, 0.45)
    ...
    def compare_13tev(self, filename_comp, comp_partner, comp_partner_hist, decay, name_plot, save):
        ...
        if comp_partner == "eff_times_acc":
            if decay == "dalitz":
                if self.meson == "pi0":
                    legend1.AddEntry(h1, "eff*acc for #pi^{0} -> e^{+} e^{-} #gamma at #sqrt{s} = 13.6TeV", "LP")
                    legend1.AddEntry(graph, "eff*acc for #pi^{0} -> e^{+} e^{-} #gamma at #sqrt{s} = 13.0TeV", "LP")
                if self.meson == "eta":
                    legend1.AddEntry(h1, "eff*acc for #eta -> e^{+} e^{-} #gamma at #sqrt{s} = 13.6TeV", "LP")
                    legend1.AddEntry(graph, "eff*acc for #eta -> e^{+} e^{-} #gamma at #sqrt{s} = 13.0TeV", "LP")
            if decay == "pcm":
                if self.meson == "pi0":
                    legend1.AddEntry(h1, "eff*acc for #pi^{0} -> #gamma #gamma at #sqrt{s} = 13.6TeV", "LP")
                    legend1.AddEntry(graph, "eff*acc for #pi^{0} -> #gamma #gamma at #sqrt{s} = 13.0TeV", "LP")
                if self.meson == "eta":
                    legend1.AddEntry(h1, "eff*acc for #eta -> #gamma #gamma at #sqrt{s} = 13.6TeV", "LP")
                    legend1.AddEntry(graph, "eff*acc for #eta -> #gamma #gamma at #sqrt{s} = 13.0TeV", "LP")
        ...
        if comp_partner == "eff_times_acc":
            h1.SetTitle("Efficiency times Acceptance")
            h1.SetXTitle("p_{T} (GeV/c)")
            h1.SetYTitle("Efficiency times Acceptance")
        x = h1.GetXaxis()
        x.SetTitleSize(0.04)
        x.SetTitleFont(42)
    ...
    def compare_cuts(self, fHistolist, cutnames, decay, filename_mc, config, name_plot, comp_partner, utils):
        ...
        if comp_partner == "eff_acc":
            FrameSettings(frame1, "", "efficiency x acceptance x BR", 0., 12.)
        if comp_partner == "eff":
        ...
        if comp_partner == "eff_acc":
            txt.AddText("Eff x acc x BR as a function of #it{p}_{T}")
        if comp_partner == "eff":
    ...
    def corr_yield_ratio(self, corr_eta, corr_pi0, name_plot, save):
        ...
        y = ratio.GetYaxis() 
        y.SetTitle("ratio #eta / #pi^{0}")
        y.SetTitleSize(0.048)
        y.SetTitleFont(42)
        y.SetLabelFont(42)
        y.SetLabelSize(0.035)
        #y.SetRangeUser(0.1, 1.5)
    ...
    def comp_eff_Teff(self, utils, raw_mc, h_eff_acc, h_Teff_acc, name_plot, save):
        h_raw = utils.scale_by_BR(raw_mc)
