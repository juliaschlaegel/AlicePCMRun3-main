...
class Compare:
    ...
    def compare_corr_dalitz_pcm(self, corr_dalitz, corr_pcm, name_plot, save): 
        ...
        c1 = ROOT.TCanvas("corr_pcm_dal", "corr_pcm_dal", 800, 600)
        c1.Divide(1, 2)
        ...
        #h1.GetXaxis().SetRangeUser(1e-3, 11)
        h1.Draw("E1P")

        h2.SetLineColor(kViolet)
        h2.SetMarkerColor(kViolet)
        h2.SetMarkerStyle(34)  
        h2.GetXaxis().SetRangeUser(0.0, 11)
        h2.Draw("E1P same")     
        ...
        ratio.SetTitle("Ratio Dalitz/PCM")
        ratio.SetXTitle("p_{T} (GeV/c)")
        ratio.GetXaxis().SetRangeUser(0.4, 11)
    ...
    def plot_eff_times_acc(self, filename_mc, config, raw_mc_dalitz, raw_mc_pcm, name_plot, save, utils):
        ...
        c1 = ROOT.TCanvas("product_eff_acc_dal_vs_pcm", "product_eff_acc_dal_vs_pcm", 800, 600)
        pad = c1.cd()
    ...
    def compare_13tev(self, filename_comp, comp_partner, comp_partner_hist, decay, name_plot, save):
        ...
        if comp_partner == "inv_yield":
            hist_comp = "InvCrossSectionPi0"
            h1 = comp_partner_hist
        ...
        if comp_partner == "inv_yield":
            c1 = ROOT.TCanvas("comp_run2_inv_yield", "comp_run2_inv_yield", 800, 600)
        ...
        if comp_partner == "inv_yield":
            if decay == "dalitz":
                if self.meson == "pi0":
                    legend1.AddEntry(h1, "invariant yield for #pi^{0} -> e^{+} e^{-} #gamma at #sqrt{s} = 13.6TeV", "LP")
                    legend1.AddEntry(graph, "invariant yield for #pi^{0} -> e^{+} e^{-} #gamma at #sqrt{s} = 13.0TeV", "LP")
                if self.meson == "eta":
                    legend1.AddEntry(h1, "invariant yield for #eta -> e^{+} e^{-} #gamma at #sqrt{s} = 13.6TeV", "LP")
                    legend1.AddEntry(graph, "invariant yield for #eta -> e^{+} e^{-} #gamma at #sqrt{s} = 13.0TeV", "LP")
            if decay == "pcm":
                if self.meson == "pi0":
                    legend1.AddEntry(h1, "invariant yield for #pi^{0} -> #gamma #gamma at #sqrt{s} = 13.6TeV", "LP")
                    legend1.AddEntry(graph, "invariant yield for #pi^{0} -> #gamma #gamma at #sqrt{s} = 13.0TeV", "LP")
                if self.meson == "eta":
                    legend1.AddEntry(h1, "invariant yield for #eta -> #gamma #gamma at #sqrt{s} = 13.6TeV", "LP")
                    legend1.AddEntry(graph, "invariant yield for #eta -> #gamma #gamma at #sqrt{s} = 13.0TeV", "LP")
    ...
    def compare_13tev_eff_acc_new(self, filename_comp, comp_partner_hist, decay, name_plot, save):
        ...
        dir_1 = filename_comp.Get("Pi013TeV") 
    ...
    def comp_eff_Teff(self, utils, h_eff_acc, h_Teff_acc, name_plot, save):
        h1 = h_eff_acc
        h2 = h_Teff_acc
        #h3 = h_eff_acc_bin
        ...
        c1 = ROOT.TCanvas("eff_vs_teff", "eff_vs_teff", 900, 800)
        ...
        h1.Draw("E1P Same") 
        h2.GetPaintedGraph().GetYaxis().SetRangeUser(5e-8,5e-4)  

        # h3.SetLineColor(ROOT.kOrange)
        # h3.SetMarkerColor(ROOT.kOrange)
        #h3.Set
        #h3.Draw("E1P Same")

        #legend = ROOT.TLegend(0.7, 0.8, 0.9, 0.7)
        legend = ROOT.TLegend(0.6, 0.3, 0.8, 0.2)
        legend.SetBorderSize(0)
        legend.SetFillColor(kWhite)
        legend.SetFillStyle(0)
        legend.SetTextSize(0.03)
        legend.AddEntry(h1, "#varepsilon x A x BR", "LP")
        legend.AddEntry(h2, "TEfficiency", "LP")
        #legend.AddEntry(h3, "#varepsilon x A x BR with binomial errors", "LP")
        legend.Draw()
        ...
        h1.SetTitle("Comparison of uncertainties")
        h1.SetXTitle("p_{T} (GeV/c)")
        h1.SetYTitle("#varepsilon x A x BR")
        h1.GetXaxis().SetRangeUser(0,12)
