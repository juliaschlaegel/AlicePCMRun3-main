...
class Efficiency:
    ...
    #     if self.meson == "eta":
    #         if self.decay == "pcm":
    #             leg.AddEntry(h_eff, "\eta \\rightarrow \gamma \gamma", "LP")
    #         else:
    #             leg.AddEntry(h_eff, "\eta \\rightarrow e^{+} e^{-} \gamma", "LP")
    #     leg.Draw("")
    ...
    #get canvas and pad to plot the acceptance in 
        ...
        y.SetTitle("efficiency x acceptance x BR") 
    ...
    def teff_and_acc_vs_pt(self, raw_mc, utils, name_plot, save):
        ...
        h_eff_acc.Draw()
        h_eff_acc.SetTitle("TEfficiency; p_{T} [GeV/c] ; Tefficiency x acceptance x BR")
        c.Update()
