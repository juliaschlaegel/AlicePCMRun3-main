...
class Efficiency:
    ...
    #get canvas and pad to plot the acceptance in 
        ...
        for i in range(0, len(pt)):
            ...
            if h_eff_acc.GetBinError(i) != 0:
                rel = float(h_eff_acc.GetBinError(i)) / float(h_eff_acc.GetBinContent(i))
        ...
        txt3 = TPaveText(0.85,0.22,0.85,0.22,"NDC"); 
    ...
    def teff_and_acc_vs_pt(self, raw_mc, utils, name_plot, save):
        ...
        def shift_histo(hist, shift):
            ...
            for i in range(1, nbins + 1):
                ...
                #print("check eff: ", hist.GetBinContent(i), "±", hist.GetBinError(i))
        ...
        teff = ROOT.TEfficiency(shifted_h1, shifted_h2)
        teff.SetStatisticOption(ROOT.TEfficiency.kFCP)
        #h_eff_acc.SetStatisticOption(ROOT.TEfficiency.kBBayesian)
        #h_eff_acc.SetStatisticOption(ROOT.TEfficiency.kBUniform)
        pt = utils.get_bin_var()
        for i in range(0, len(pt)):
            print("Tefficiency: ",i, teff.GetEfficiency(i),teff.GetEfficiencyErrorLow(i),teff.GetEfficiencyErrorUp(i))
        for i in range(2,len(pt)):
            if teff.GetEfficiency(i) != 0:
                print("relative error for TEfficiency: ", teff.GetEfficiencyErrorLow(i)/teff.GetEfficiency(i), teff.GetEfficiencyErrorUp(i)/teff.GetEfficiency(i))
        ...
        teff.Draw()
        teff.SetTitle("TEfficiency; p_{T} [GeV/c] ; #varepsilon x A x BR")
        c.Update()
        
        # Define marker settings
        teff.SetFillColor(kCyan+1)
        teff.SetMarkerStyle(kFullCross)
        teff.SetMarkerColor(kBlue)
        teff.SetMarkerSize(1.5)
        ...
        if self.meson == "pi0":
            if self.decay == "pcm":
                leg.AddEntry(teff, "\pi^{0} \\rightarrow \gamma \gamma", "LP")
            else:
                leg.AddEntry(teff, "\pi^{0} \\rightarrow e^{+} e^{-} \gamma", "LP")
        if self.meson == "eta":
            if self.decay == "pcm":
                leg.AddEntry(teff, "\eta \\rightarrow \gamma \gamma", "LP")
            else:
                leg.AddEntry(teff, "\eta \\rightarrow e^{+} e^{-} \gamma", "LP")
        leg.Draw("")
        ROOT.SetOwnership(leg,False)
        ...
        print("eff and acc hist type:", type(teff))
        return teff
    
    def eff_and_acc_vs_pt_bin(self, scale_BR, raw_mc, utils, name_plot, save):
        #Get wanted histograms:
        h1 = raw_mc
        h2 = utils.rebin_hist("hPt")
        utils.scale_by_Nev(h2)
    ...
    #get canvas and pad to plot the acceptance in 
        can = TCanvas("eff_acc_bin", "eff_acc_bin", 0, 0, 900, 900)
        pad = can.cd()
        pad.SetPad(0.0, 0.01, 1, 1)
        pad.SetMargin(0.15, 0.1, 0.1, 0.1)
        pad.SetTicks(1,1)
        ...
        pt = utils.get_bin_var()
        shifted_pt = [x + shift for x in pt]

        
        h_eff_bin = ROOT.TH1F("eff_bin", "eff_bin", len(shifted_pt)-1, np.array(shifted_pt))
    
        h_eff_bin.Divide(shifted_h1, shifted_h2, 1.0, 1.0, "B")
        
        
        if scale_BR == "true":
            utils.scale_by_BR(h_eff_bin)
        
        # for i in range(0, len(pt)):
        #     print("###########Efficiency, values and Errors" , pt[i] ,"########")
        #     print("Value of raw mc: ", h1.GetBinContent(i), "±", h1.GetBinError(i))
        #     print("Value of hPt all: ", h2.GetBinContent(i), "±", h2.GetBinError(i))
        #     print("value of eff x acc x BR: ", h_eff_bin.GetBinContent(i), "±", h_eff_bin.GetBinError(i))
        #     if h_eff_bin.GetBinError(i) != 0:
        #         rel = float(h_eff__bin.GetBinError(i)) / float(h_eff_bin.GetBinContent(i))
        #         print("relative error: ", rel)
        
        # Define marker settings
        h_eff_bin.SetFillColor(kCyan+1)
        if self.decay == "pcm":
            h_eff_bin.SetMarkerStyle(34)
        if self.decay == "dalitz":
            h_eff_bin.SetMarkerStyle(20)
        h_eff_bin.SetMarkerColor(kBlue)
        h_eff_bin.SetMarkerSize(1.5)
        h_eff_bin.SetLineColor(kBlue)

        #Draw points in the histogramm
        h_eff_bin.Draw("Esame")
       
        # Define y-axis settings
        y = h_eff_bin.GetYaxis() 
        y.SetTitle("#varepsilon x A x BR") 
        y.SetTitleSize(0.025)
        y.SetTitleFont(42)
        y.SetLabelFont(42)
        ...
        if self.meson == "pi0":
            if self.decay == "pcm":
                leg.AddEntry(h_eff_bin, "\pi^{0} \\rightarrow \gamma \gamma", "LP")
            else:
                leg.AddEntry(h_eff_bin, "\pi^{0} \\rightarrow e^{+} e^{-} \gamma", "LP")
        if self.meson == "eta":
            if self.decay == "pcm":
                leg.AddEntry(h_eff_bin, "\eta \\rightarrow \gamma \gamma", "LP")
            else:
                leg.AddEntry(h_eff_bin, "\eta \\rightarrow e^{+} e^{-} \gamma", "LP")
        leg.Draw("")
        ROOT.SetOwnership(leg,False)
        ...
        if save == "true":
            can.SaveAs(name_plot.Data())
        print("eff and acc hist type:", type(h_eff_bin))
        return h_eff_bin