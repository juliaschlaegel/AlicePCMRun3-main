...
class PairAnalyzer:
    ...
    def __init__(self, meson, filename, dirname, type, filename_mc, config, decay):
        print("used config_file: ", config)
        print("target meson = {0} , filename = {1} , dirname = {2}".format(meson, filename, dirname))
        self.meson = meson;
        self.type = type
        self.filename_mc = filename_mc
    ...
    def calculate_raw_yield(self, histo, params, params_err, binwidth, nev):
        ...
        linear_integral = linear_integral / histo.GetBinWidth(2)
        #linear_integral.Scale(1/histo.GetBinWidth(2))
        print("linear integral: ", linear_integral)
        ...
        #error.Scale(1/histo.GetBinWidth(2))
        #error = error /histo.GetBinWidth(2)
        error = error  /binwidth /nev
    ...
    def set_2D_histograms(self, outlist, ssname):
        ...
        h2same = self.get_2D_histo_same(ssname,"same")

        h2mix = self.get_2D_histo_same(ssname, "mix")
        ...
        h2same.Sumw2();
        h2mix.Sumw2();
        utils = Utility(self.meson, self.filename, self.config, self.decay, "data")
        nev = utils.get_Nev()
    ...
    def set_2D_histograms_MC(self, outlist, ssname_mc):
        ...
        #     if self.meson == "eta":
        #         h2mc_help = self.list_pair.Get("Eta")
        #     if h2mc_help:
        #         h2mc = h2mc_help.Get("hMggPt_Primary").Clone("h2mc");
        #     else:
        #         print("Object 'Pi0' or eta not found in list_pair_ss_cut.")
        # else:
        #     print("list_pair_ss_cut is not valid.")
        
        h2mc = self.get_2D_histo_primary(ssname_mc)
        ...
        if self.meson == "pi0":      
            ...
            h2mc.RebinX(4);
        h2mc.Sumw2();
        utils = Utility(self.meson, self.filename_mc, self.config, self.decay, "mc")
        nev = utils.get_Nev()
        #nev = h1ev.GetBinContent(7);
        print("NEV = ", nev)
    ...
    def analyze_ptspectrum(self, start_pT, ssname):
        outlist = THashList();
        outlist.SetName("outlist");
        npt = len(self.arr_pt);
        
        if self.type == "data":
            h2same, h2mix, h1ev, nev = self.set_2D_histograms(outlist, ssname)
        elif self.type == "mc":
            h2mc, h1ev, nev = self.set_2D_histograms_MC(outlist, ssname)
        ...
        for i in range(start_pT, npt-1):
            ...
            f1total.SetRange(fit_min, fit_max)
            ...
            for i_lim, (lower_limit, upper_limit) in enumerate(zip(self.initial_lower_lim, self.initial_upper_lim)):
                ...
                # print(i_lim, "lower limit: ", lower_limit)
            ...
            h1fit.Fit(f1total,"R","",fit_min, fit_max); #RME
