    ...
    if config["common"]["do_ptspectrum"] == True:
        if run_type == "one":
            ...
            # pdf output of all fitted histograms for each combination of cut and fit
                        ...
                        plot_pi0.PlotInvMassInPtBins(utils, start_pt, histogram_list, function_list, TString(output_name_with_yield), "", "", 
                                                    plottingRange, period, 3, 4, 0, len(arr_pt),len(arr_pt), "#gamma#gamma", 
            ...
            # pdf output of all same and mixed scaled histograms for each cut
                        if typ == "data":
                            ...
                            plot_same_mixed.PlotSameMixedInPtBins(start_pt, same_list, mixed_list, TString(output_name), "", "", plottingRange, period, 3, 4, 0, 
        ...
        if run_type == "both_types":
            ...
            for isys in range(0, nsys):
                ...
                for i in range(1, eff_acc.GetNbinsX() + 1):
                    ...
                    print(f"Bin {i}: Center = {bin_center_d}, Content = {bin_content_d}")
                if decay == "pcm":
                    eff_acc_list_pcm.append(copy.deepcopy(eff_acc))
                    if meson == "eta":
                        filename_eff_acc_pcm_eta = f"eff_pcm_eta_{isys}"
                        with open(filename_eff_acc_pcm_eta, "wb") as eff_pcm_file_eta:
                            pickle.dump(eff_acc, eff_pcm_file_eta)
                    if meson == "pi0":
                        filename_eff_acc_pcm_pi0 = f"eff_pcm_pi0_{isys}"
                        with open(filename_eff_acc_pcm_pi0, "wb") as eff_pcm_file_pi0:
                            pickle.dump(eff_acc, eff_pcm_file_pi0)
                    save_hist.add_histogram("PCMPCM", eff_acc)
                    save_hist.save_histograms()
                if decay == "dalitz":
                    eff_acc_list_dalitz.append(copy.deepcopy(eff_acc))
                    if meson == "eta":
                        filename_eff_acc_dalitz_eta = f"eff_dalitz_eta_{isys}"
                        with open(filename_eff_acc_dalitz_eta, "wb") as eff_dalitz_file_eta:
                            pickle.dump(eff_acc, eff_dalitz_file_eta)
                    if meson == "pi0":
                        filename_eff_acc_dalitz_pi0 = f"eff_dalitz_pi0_{isys}"
                        with open(filename_eff_acc_dalitz_pi0, "wb") as eff_dalitz_file_pi0:
                            pickle.dump(eff_acc, eff_dalitz_file_pi0)
                ...
                if decay == "dalitz":
                    ...
                    corr_name= os.path.join(folder, "inv_yield_calculation", "Dalitz", "{0}_corrected_yield_{1}".format(date,ssname))
                    corr_name_scaled = os.path.join(folder, "inv_yield_calculation", "Dalitz", "{0}_corrected_yield_scaled_by_pT".format(date))
                    comp_pythia_corr_name = os.path.join(folder, "inv_yield_calculation",  "Dalitz", "{0}_comp_pythia_corr_yield_{1}".format(date,ssname))
                else:                    
                ...
                if decay == "dalitz":
                    ...
                    save_hist.save_histograms()
                    print("List corr_yield_dalitz: ", corr_list_dalitz)
                ...
                comp_inv_yield_13Tev_dalitz_name = os.path.join(folder, "Comparison", "Comp_13TeV_inv_yield_dalitz")
                comp_inv_yield_13Tev_pcm_name = os.path.join(folder, "Comparison", "Comp_13TeV_inv_yield_pcm")
                ...
                for id in range(0, len(eff_acc_list_dalitz)):
                    ...
                    print(ssname_cut)
                    comp_eff_times_acc_13Tev_calc_dalitz = os.path.join(folder, "Comparison", "Comp_new_13TeV_eff_acc_dalitz_{0}".format(ssname_cut))
                    ...
                    if meson == "pi0":
                        ...
                        comp_class.compare_13tev_eff_acc_new(filename_comp_dalitz, eff_acc_list_dalitz[id], "dalitz", TString(comp_eff_times_acc_13Tev_calc_dalitz), "true")

                for ip in range (0, len(eff_acc_list_pcm)):
                    ssname_cut = config[typ]["subsystems"][ip]["name"]
                    print(ssname_cut)
                    ssname_list_pcm.append(ssname_cut)
                    comp_eff_times_acc_13Tev_calc_pcm = os.path.join(folder, "Comparison", "Comp_new_13TeV_eff_acc_pcm_{0}".format(ssname_cut))
                    ...
                    if meson == "pi0":
                        ...
                        comp_class.compare_13tev_eff_acc_new(filename_comp_pcm, eff_acc_list_pcm[ip], "pcm", TString(comp_eff_times_acc_13Tev_calc_pcm), "true")
                ...
                name_eff_acc_comp = os.path.join(folder, "Comparison", "comp_eff_acc_pcm_dalitz")
                comp_class.plot_eff_acc_dalitz_vs_pcm(filename_mc, config, raw_mc_dalitz, raw_mc_pcm, TString(name_eff_acc_comp), "true")

                inv_yield = utils_mc.scale_to_inv_yield(corr_scaled)

                
                if decay == "dalitz":
                    if meson == "pi0":
                        comp_class.compare_13tev(filename_comp_dalitz, "corr", corr_scaled, "dalitz", TString(comp_corr_13Tev_dalitz_name), "true")
                        comp_class.compare_13tev(filename_comp_dalitz, "inv_yield", inv_yield, "dalitz", TString(comp_inv_yield_13Tev_dalitz_name), "true")
                        
                if decay == "pcm":
                    if meson == "pi0":
                        comp_class.compare_13tev(filename_comp_pcm, "corr", corr_scaled, "pcm", TString(comp_corr_13Tev_pcm_name), "true")
                        comp_class.compare_13tev(filename_comp_pcm, "inv_yield", inv_yield, "pcm", TString(comp_inv_yield_13Tev_pcm_name), "true")
        ...
        if run_type == "both_mesons":
            ...
            if meson == final_meson:
                ...
                filename_eff_pcm_eta = f"eff_pcm_eta_0"
                with open(filename_eff_pcm_eta, "rb") as eff_pcm_eta_file:
                    eff_acc_pcm_eta = pickle.load(eff_pcm_eta_file)
                    print("eff acc pcm eta loaded: ", eff_acc_pcm_eta, type(eff_acc_pcm_eta))
                filename_eff_pcm_pi0 = f"eff_pcm_pi0_0"
                with open(filename_eff_pcm_pi0, "rb") as eff_pcm_pi0_file:
                    eff_acc_pcm_pi0 = pickle.load(eff_pcm_pi0_file)
                    print("eff acc pcm eta loaded: ", eff_acc_pcm_pi0, type(eff_acc_pcm_pi0))
                filename_eff_dalitz_eta = f"eff_dalitz_eta_0"
                with open(filename_eff_dalitz_eta, "rb") as eff_dalitz_eta_file:
                    eff_acc_dalitz_eta = pickle.load(eff_dalitz_eta_file)
                    print("eff acc dalitz eta loaded: ", eff_acc_dalitz_eta, type(eff_acc_dalitz_eta))
                filename_eff_dalitz_pi0 = f"eff_dalitz_pi0_0"
                with open(filename_eff_dalitz_pi0, "rb") as eff_dalitz_pi0_file:
                    eff_acc_dalitz_pi0 = pickle.load(eff_dalitz_pi0_file)
                    print("eff acc pcm eta loaded: ", eff_acc_dalitz_pi0, type(eff_acc_dalitz_pi0))
                ...
                name_eff_acc_pcm = os.path.join(folder, "Comp_eff_acc_eta_pi0_pcm")
                name_eff_acc_dalitz = os.path.join(folder, "Comp_eff_acc_eta_pi0_dalitz")
                h_eta_pi0 = comp_class.corr_yield_ratio(corr_yield_pcm_eta, corr_yield_pcm_pi0, TString(name_ratio), "true")
                print(" type of eta to pi0 ratio hist: ", type(h_eta_pi0))
                comp_class_eta.compare_13tev_etapi0(filename_comp_pcm, h_eta_pi0, TString(name_comp_13Tev), "true")
                comp_class.compare_eff_eta_pi0(eff_acc_pcm_eta, eff_acc_pcm_pi0, "pcm", TString(name_eff_acc_pcm), "true")
                comp_class.compare_eff_eta_pi0(eff_acc_dalitz_eta, eff_acc_dalitz_pi0, "dalitz", TString(name_eff_acc_dalitz), "true")
...
filename_array = [os.path.join(myDir,"HLtrains/data/AnalysisResults_HL233027_LHC22o_pass6.root"),os.path.join(myDir,"HLtrains/MC/AnalysisResults_HL233297_LHC24b1.root")]

filename_data = os.path.join(myDir,"HLtrains/data/AnalysisResults_HL233027_LHC22o_pass6.root")
filename_mc = os.path.join(myDir,"HLtrains/MC/AnalysisResults_HL233297_LHC24b1.root")
...
type_array = ["data", "mc"] # gives the type (data or mc) for the data in the filename_array
decay_array = ["dalitz", "pcm"] # gives the decay modes that shall be analysed
run_type_array = ["one", "both_types", "both_decays", "both_mesons"] # gives what part of the analysis shall be done 
#one: invariant mass analysis, both types: invariant yield analysis where mc and data are needed simultaniously, 
#both decays: for comparing between the two decay modes, both mesons: for comparing between the mesons (can only be done if more than one meson is given)
meson_array = ["eta", "pi0"] 
start_pt_array = ["false"] #, "true"] # gives the starting of the pT, "false": the pT starts in the first given bin from the config,
# "true": tells the script to start at the pT start, which is set in the config, instead of the first bin


for mes in range(len(meson_array)):
    meson = meson_array[mes]
    if meson == "pi0":
        config_array_pcm = [os.path.join(myDir,"AlicePCMRun3-main/PhotonMomentumResolution_new_output/configs/config_pp_13.6TeV_pi0_LHC22o_full_pcm.yml"), os.path.join(myDir,"AlicePCMRun3-main/PhotonMomentumResolution_new_output/configs/config_pp_13.6TeV_pi0_LHC24b1_full_pcm.yml")]
        config_array_dalitz = [os.path.join(myDir,"AlicePCMRun3-main/PhotonMomentumResolution_new_output/configs/config_pp_13.6TeV_pi0_LHC22o_full_dalitz.yml"), os.path.join(myDir,"AlicePCMRun3-main/PhotonMomentumResolution_new_output/configs/config_pp_13.6TeV_pi0_LHC24b1_full_dalitz.yml")]
        #config_array_dalitz = [os.path.join(myDir,"AlicePCMRun3-main/PhotonMomentumResolution/configs/config_test"), os.path.join(myDir,"AlicePCMRun3-main/PhotonMomentumResolution/configs/config_pp_13.6TeV_pi0_LHC24b1_full_dalitz.yml")]
    if meson == "eta":  
        config_array_pcm = [os.path.join(myDir,"AlicePCMRun3-main/PhotonMomentumResolution_new_output/configs/config_pp_13.6TeV_eta_LHC22o_full_pcm.yml"), os.path.join(myDir,"AlicePCMRun3-main/PhotonMomentumResolution_new_output/configs/config_pp_13.6TeV_eta_LHC24b1_full_pcm.yml")]
        config_array_dalitz = [os.path.join(myDir,"AlicePCMRun3-main/PhotonMomentumResolution_new_output/configs/config_pp_13.6TeV_eta_LHC22o_full_dalitz.yml"), os.path.join(myDir,"AlicePCMRun3-main/PhotonMomentumResolution_new_output/configs/config_pp_13.6TeV_eta_LHC24b1_full_dalitz.yml")]
        ...
        for dec in range(len(decay_array)):
            ...
            for r in range(len(run_type_array)):
                ...
                for i in range(len(period_array)):
                    ...
                    folder = "/Users/juliaschlagel/Analysis240411/Analysis/AlicePCMRun3-main/analysis_results_new_output_{0}".format(meson)
